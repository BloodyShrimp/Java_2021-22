...
class stolen implements CompressionInterface {
    ...
    public void compress() {
        ...
        if (optymalny < dlugoscWejscia) {
            ...
            for (int i = 0; i < optJ; i++) {
                naglowek.put(String.format("%" + optI + "s", Integer.toBinaryString(i)).replaceAll(" ", "0"),
                        liczbaSlowMalejaco.keySet().toArray()[i].toString());
            }

            naglowekOdwrotny = naglowek.entrySet().stream()
